...
const ServiceDBSalesHistory = () => {
  const { service_name, date } = useParams();
  ...
  useEffect(() => {
    const fetchDailySalesSummary = async () => {
      try {
        ...
        const res = await api.get(`/service/sales-db-summary/${encodedServiceName}?date=${date}`);
  ...
  return (
    <div className="p-6 bg-white rounded-md shadow-md">
      ...
      <h2 className="text-2xl font-bold mb-4">
        Daily Breakdown of Sales History for "{decodeURIComponent(service_name)}" on {date}
      </h2>
        ...
        <table className="w-full border-collapse border mb-6 bg-gray-50 rounded-lg overflow-hidden shadow-sm">
          ...
          <tbody>
            <tr className="text-center bg-white">
              <td className="border p-3">{summary.service_name}</td>
              <td className="border p-3">{summary.service_category_name}</td>
...
export default ServiceDBSalesHistory;
