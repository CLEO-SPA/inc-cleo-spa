...
const ProductMonthlySalesHistory = () => {
  ...
  const fetchProducts = async () => {
    try {
      const res = await api.get(`/p/getAllProO`);
  ...
  const fetchSalesSummary = async () => {
    ...
    try {
      ...
      const res = await api.get(`/p/sales-summary/${encodedProductName}?month=${formattedMonth}&year=${year}`);
  ...
  return (
    <div className="p-6 bg-white rounded-md shadow-md">
      <h2 className="text-2xl font-bold mb-4">Product Monthly Sales History</h2>
        ...
        <table className="w-full border-collapse border mb-6">
          ...
          <tbody>
            <tr className="text-center">
              <td className="border p-2">{selectedProduct || 'N/A'}</td>
              <td className="border p-2">{monthlySummary?.detailed_sales[0]?.product_category_name || 'N/A'}</td>
                ...
                <td className="border p-2">
                  <button
                    onClick={() => {
                      ...
                      navigate(`/psh/db/${encodeURIComponent(selectedProduct)}/${formattedDate}`);
                    }}
