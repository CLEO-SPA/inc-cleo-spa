...
const ProductRevenueDetails = ({ selectedDate, onClose }) => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            try {
                setIsLoading(true);
                setError(null);
                const response = await productRevenueApi.getDetailsByDate(selectedDate);
                if (response.success) {
            ...
            <div className="bg-[#1f2937] rounded-lg w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
                <div className="p-4 border-b border-gray-700 flex justify-between items-center">
                    <h2 className="text-xl font-semibold text-gray-100">
                        Product Revenue Details - {selectedDate && formatDate(selectedDate)}
                    </h2>
                    <button
                        onClick={onClose}
...
export default ProductRevenueDetails;