...
const CartItemsWithPayment = ({
  ...
  const calculateItemsFreeAmount = (items) => {
    ...
    items.forEach((item) => {
      ...
      if (item.type === 'member-voucher' || item.type === 'transferMCP' || item.type === 'transferMV') {
        return;
      }
  ...
  const getPaymentSections = () => {
    ...
    if (servicesAndProducts.length > 0) {
      const combinedAmount = roundTo2Decimals(
  ...
  const updatePaymentAmount = (sectionId, paymentId, amount) => {
    ...
    const otherNonGSTNonFreePaymentsTotal = roundTo2Decimals(
      currentPayments
    ...
    if (section?.isDynamicGST) {
      ...
      if (numAmount > maxAllowed && maxAllowed >= 0) {
        console.warn(
          `Payment amount limited from ${numAmount} to ${clampedAmount} for dynamic GST section ${sectionId}`
        );
      }
    } else {
  ...
  const autoAddTransferPayment = (sectionId, transferAmount, isPartialTransfer = false) => {
    ...
    const existingPayments = sectionPayments[sectionId] || [];
    const hasNonGSTNonFreePayments = existingPayments.some(
      (payment) => payment.methodName !== `GST (${gstRate}%)` && payment.methodName !== 'Free'
    );
      ...
      if (newFreeAmount > 0) {
        ...
        if (existingFreePayment) {
          ...
          if (roundTo2Decimals(existingFreePayment.amount) !== newFreeAmount) {
            console.log(
              `Updating Free amount for ${section.id} from ${existingFreePayment.amount} to ${newFreeAmount}`
            );
            onPaymentChange('updateAmount', section.id, {
    ...
    paymentSections.forEach((section) => {
      if (section.id.startsWith('transfer-mcp-') || section.id.startsWith('transfer-mv-')) {
        ...
        existingPayments.forEach((payment) => {
          if (
            (payment.methodName === 'Transfer' || payment.methodId === 9) &&
            payment.amount === 0 &&
            !payment.isGST &&
            !payment.isFree
          ) {
            ...
            <div className='flex items-center justify-between'>
              ...
              <div className='text-right'>
                ...
                <div className='text-lg font-bold text-blue-600'>
                  Total with GST: {formatCurrency(section.totalWithGST)}
                </div>
              </div>
            </div>
          </CardHeader>
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className='border-t border-gray-200 hover:bg-gray-50'>
                        ...
                        <td className='px-4 py-3 text-right text-gray-900'>{formatCurrency(pricing.originalPrice)}</td>
                        ...
                        <td className='px-4 py-3 text-right'>
                          {itemFreeAmount > 0 ? (
                            <span className='text-green-600 font-medium'>{formatCurrency(itemFreeAmount)}</span>
                          ) : (
                  ...
                  <tr>
                    ...
                    <td className='px-4 py-3 text-right'>
                      {section.freeAmount > 0 ? (
                        <span className='text-green-600 font-bold text-lg'>{formatCurrency(section.freeAmount)}</span>
                      ) : (
                        <span className='text-gray-400'>-</span>
                      )}
                    </td>
              ...
              {sectionPayments[section.id] && sectionPayments[section.id].length > 0 && (
                <div className='space-y-3'>
                  {sectionPayments[section.id].map((payment) => {
                    ...
                    const otherNonGSTNonFreePaymentsTotal = roundTo2Decimals(
                      sectionPayments[section.id]
                        .filter(
                          (p) => p.id !== payment.id && p.methodName !== `GST (${gstRate}%)` && p.methodName !== 'Free'
                        )
                        .reduce((sum, p) => sum + p.amount, 0)
                    );
                      ...
                      >
                        <div className='flex-shrink-0 w-32'>
                          ...
                          >
                            ...
                            {isAutoGeneratedFree && (
                              <span className='block text-xs text-green-600'>Auto-calculated</span>
                            )}
                        ...
                        <div className='flex-1'>
                          ...
                          {!isGST && !isAutoGeneratedFree && maxAllowed <= 0 && (
                            <div className='text-xs text-orange-500 mt-1'>Section fully paid</div>
                          )}
                        </div>
                        <div className='flex-1'>
                          <input
                ...
                <div className='space-y-2'>
                  ...
                  {section.freeAmount > 0 && (
                    <div className='flex justify-between items-center'>
                      ...
                      <span className='font-bold text-lg text-green-600'>{formatCurrency(section.freeAmount)}</span>
                    </div>
                  )}
                  <div className='flex justify-between items-center'>
                    <span className='font-medium'>GST ({gstRate}%):</span>
                    <span className='font-bold text-lg text-blue-600'>{formatCurrency(section.gstAmount)}</span>
                  </div>
