...
export function MembershipTypeSelect() {
  ...
  useEffect(() => {
    if (membershipTypeList.length === 0 && !loading) {
      fetchAllMembershipType();
    }
  }, [membershipTypeList.length, loading, fetchAllMembershipType]);

  return (
    <div className="space-y-2">
      <Label
        htmlFor="membership_type_id"
      ...
      {errors.membership_type_id && (
        <p className="text-red-500 text-xs">{errors.membership_type_id.message}</p>
      )}
      
      {/* Store error message */}
      {error && (
        <p className="text-red-500 text-xs">Failed to load membership types: {error}</p>
      )}
    </div>
  );
}
