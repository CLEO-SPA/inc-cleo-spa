...
const getMVMonthlyReportUpdated = async (year: number, month: number) => {
  const client = await pool().connect();
  try {
    const result1 = await client.query(
      `
      SELECT * FROM get_mv_income_by_month_updated($1, $2);
      `,
      [year, month]
    );
...
const getMCPMonthlyReportUpdated = async (year: number, month: number) => {
  const client = await pool().connect();
  try {
    const result1 = await client.query(
      `
      SELECT * FROM get_mcp_income_by_month_updated($1, $2);
      `,
      [year, month]
    );
...
const getAdHocMonthlyReportUpdated = async (year: number, month: number) => {
  const client = await pool().connect();
  try {
    const result1 = await client.query(
      `
      SELECT * FROM get_adhoc_income_by_month_updated($1, $2);
      `,
      [year, month]
    );
...
const getMonthlyIncomeTest = async (year: number, month: number) => {
  const client = await pool().connect();
  try {
    const result1 = await client.query(
      `
