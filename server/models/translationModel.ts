...
const addTranslation = async (
  ...
  try {
    const existingCheck = await dbQuery('SELECT * FROM translations WHERE LOWER(english) = LOWER($1)', [english]);
    ...
    const result = await dbQuery(
      `INSERT INTO translations (
            ...
            RETURNING *`,
      [english, chinese, meaningInEnglish, meaningInChinese, timestamp]
    );
...
const deleteTranslation = async (id: number): Promise<{ message?: string; error?: string }> => {
  try {
    const existing = await dbQuery('SELECT * FROM translations WHERE id = $1', [id]);

    if (existing.rows.length === 0) {
      return { error: 'Translation not found.' };
    }

    await dbQuery('DELETE FROM translations WHERE id = $1', [id]);
...
const updateTranslation = async (
  ...
  try {
    ...
    const result = await dbQuery(`UPDATE translations SET ${setClause} WHERE id = $${fields.length + 1} RETURNING *`, [
      ...values,
      id,
    ]);
